...
// ProcessNewNZBs scans the metadata directory for unmanaged NZB files, claims
// them, parses them, and returns completed NZB metadata for manager sync.
func (u *Usenet) ProcessNewNZBs(ctx context.Context) ([]*storage.NZB, error) {
	...
	newNZBs := make([]*storage.NZB, 0, len(pending))
	for _, item := range pending {
		nzb, err := u.processClaimedNZB(ctx, item)
		if err != nil {
			u.logger.Error().Err(err).Str("filename", item.Name).Msg("NZB processing failed")
			continue
		}
		if nzb != nil {
			newNZBs = append(newNZBs, nzb)
		}
	}
	return newNZBs, nil
}

func (u *Usenet) processClaimedNZB(ctx context.Context, item PendingNZB) (*storage.NZB, error) {
	markFailed := func(err error) {
		failedPath := item.Path + ".failed"
		_ = os.WriteFile(failedPath, []byte(err.Error()), 0644)
	}

	if err := validateNZB(item.Content); err != nil {
		err = fmt.Errorf("invalid NZB content: %w", err)
		markFailed(err)
		return nil, err
	}

	nzb, groups, err := u.Parse(ctx, item.Name, item.Content, "")
	if err != nil {
	...
	nzb, err = u.Process(ctx, nzb, groups)
	if err != nil {
