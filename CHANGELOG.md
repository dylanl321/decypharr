...
- **Provider preference (with fallback):** New global `torrent_debrid_order` and `nzb_debrid_order` lists let you express a *preferred* try order per protocol while still falling back to other eligible providers on failure (e.g. DMCA / 451). Unlike `torrent_debrid` (which pins), preference still tries every eligible provider; unlisted providers fall back in physical config order. `prefer_cached_provider` continues to promote cached providers above the preference list.
- **DMCA / slot exhaustion error mapping:** The qBittorrent `add` endpoint now returns HTTP 451 for DMCA blocks (so Sonarr/Radarr mark the release as failed and try another release) and 503 with `Retry-After: 30` for slot exhaustion (so the *arr backs off instead of treating it as terminal). The SABnzbd `addurl` / `addfile` endpoints get the same status mapping, swapping the previous blanket 500.
...
- **Config UI:** New per-debrid checkboxes (Allow torrents / Allow NZBs / Remove on complete) and `max_active_downloads` field; new global "Cleanup on complete" section, torrent debrid pin, and provider preference inputs under Reliability & Queue.
